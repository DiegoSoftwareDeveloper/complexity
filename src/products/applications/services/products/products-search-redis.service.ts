...
export class ProductsSearchRedisService {
  ...
  private async createIndex() {
    try {
      await this.redisClient.call('FT.CREATE', this.INDEX_NAME, 'ON', 'HASH', 'PREFIX', '1', 'product:', 'SCHEMA',
  ...
  async searchProducts(query: string, filters: { category?: string; location?: string }, pagination: { offset: number; limit: number }): Promise<any[]> {
    ...
    const rawResults = await this.redisClient.call('FT.SEARCH', this.INDEX_NAME, searchQuery, 
      'LIMIT', pagination.offset.toString(), pagination.limit.toString(),
  ...
  async getSuggestions(query: string, limit = 5): Promise<string[]> {
    return this.redisClient.call('FT.SUGGET', this.SUGGESTION_KEY, query, 
      'FUZZY', 'MAX', limit.toString()
    ) as Promise<string[]>;
  }
